...
8. **匯出**為 JSON、SKILL.md（供 coding agent 使用）、DESIGN.md（供 Google Stitch 使用）或獨立 HTML
...
圖片上傳
  ...
  → UI Kit 渲染（25 種元件類型）
  → 匯出（JSON / SKILL.md / DESIGN.md / HTML）
```
...
- [ ] **Figma 匯出（code.to.design）** — 整合 [code.to.design](https://code.to.design/) API，將產出的 design token 與元件直接推送至 Figma
