...
async function callProvider(env, providerKey, imageBase64, prompt, maxTokens = 4096) {
  ...
  switch (p.type) {
    case "anthropic":
      return callAnthropic(env, providerKey, imageBase64, prompt, maxTokens);
    case "openai":
      return callOpenAI(env, providerKey, imageBase64, prompt, maxTokens);
    case "gemini":
      return callGemini(env, providerKey, imageBase64, prompt, maxTokens);
    default:
...
__name(checkAndIncrementRate, "checkAndIncrementRate");
async function upsertEmail(db, email, ip) {
  ...
  if (existing) {
    await db.prepare("UPDATE emails SET last_used = datetime('now'), total_uses = total_uses + 1, last_ip = ? WHERE email = ?").bind(ip, emailNorm).run();
  } else {
    await db.prepare("INSERT INTO emails (email, last_ip) VALUES (?, ?)").bind(emailNorm, ip).run();
  }
  console.log(`[worker] D1 email upsert: ${emailNorm}`);
}
__name(upsertEmail, "upsertEmail");
...
__name(saveDesignTokens, "saveDesignTokens");
var worker_rate_limit_default = {
    ...
    if (url.pathname === "/api/upload-image" && request.method === "POST") {
      ...
      try {
        ...
        await env.IMAGES.put(imageKey, binaryData, {
          ...
          customMetadata: { email: email.toLowerCase().trim(), uploadedAt: (/* @__PURE__ */ new Date()).toISOString(), ip }
        });
        if (env.DB) {
...
export {
  worker_rate_limit_default as default
};
//# sourceMappingURL=worker-rate-limit.js.map
