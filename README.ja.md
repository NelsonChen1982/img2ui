...
├── views/         # ルートページ（WizardView、GalleryList、GalleryDetail、MyDesigns）
...
worker/            # Cloudflare Worker（認証、クレジット、レート制限、R2画像ストレージ、D1、Gallery API）
docs/              # 設計ドキュメント
```
...
| バージョン | マイルストーン |
|-----------|-------------|
| **v0.3-beta** | コミュニティギャラリー（`/gallery`）：公開デザイン閲覧、カラースウォッチカード＋ホバーUIキットプレビュー、色系/テーマ/ソートフィルター、ユーザーごと最大5件表示。ギャラリー詳細ページ：カバーヒーロー、フルUIキットレンダー、折りたたみJSON、オリジナル画像表示（所有者のみ）。マイデザインページ（`/my-designs`）：公開/非公開切替、ソフト削除。トップページマーキーで最新作品を表示。vue-router統合。ダウンロードポップオーバー全6形式対応。Gallery API、R2画像プロキシ、soft delete。 |
...
- [x] **コミュニティギャラリー** — 他のユーザーが作成したデザインシステムを閲覧・探索
- [ ] **デザイン共有** — URL、埋め込みカード、SNS経由でデザインを共有（OG画像生成）
- [ ] **ランディングページ刷新** — 機能ハイライト、ユーザーの声、コンバージョンファネルを含む新デザイン
